...
- The final finish reason is attached to the assistant message as metadata, so the UI can tell how the answer ended: `stop` is a normal answer; `length` gets a short note that the answer was cut off, since the model cannot add that note itself; `content-filter`, `error`, `other` and `unknown` make it a failed turn (see "Layout and copy"), so a filtered or failed generation is never shown as a complete answer.
...
- Allowed parts: user messages contain only `text` parts; assistant messages only `text`, `step-start` and `tool-request_meeting` parts, the latter in any state (`input-streaming`, `input-available`, `output-available`, `output-error`), so a failed or cut-off call never blocks the next message. Anything else is rejected. Before `convertToModelMessages`, the server cleans the history in this order:
  ...
  3. Drop assistant messages left with no `text` and no tool part. Two user messages can then follow each other, which the model API accepts: consecutive turns of the same role are merged.
- **Rejected before any model call:**
  ...
  - HTTP 400 `{ error: "invalid_request" }` from body validation.
- Errors during the stream reach the client only as a generic error: the server's `onError` maps every error to a fixed message and never forwards provider or stack details.
- **Meeting state:** a conversation has a successful send when an assistant message in the history contains a `tool-request_meeting` part in state `output-available` with output `{ ok: true }`. The server derives this from the history at the start of each request (system prompt item 4 and the tool's `already_sent`).
...
- [ ] Finish reason `content-filter`, `error`, `other` or `unknown` (simulated) → failed turn with retry, answer left out of the next request's history?
