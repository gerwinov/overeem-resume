...
| Model | `anthropic/claude-sonnet-5`, thinking off, fixed in code | Accurate with the facts in the content; see "Model choice" |
...
`anthropic/claude-sonnet-5`, at $2 / $10 per million input/output tokens, with thinking off. Chosen for accuracy with the content. On 2026-09-13 the "Content" questions of the test plan ran against both Sonnet 5 and `anthropic/claude-haiku-4.5` ($1 / $5), with the same prompt and content:
- Haiku made factual errors about Gerwin in 5 of 25 answers (mixing up employers, years and projects; on a pasted vacancy it even said Docker and CI/CD were not in the CV) and ignored behaviour rule 4 on vacancies. Sonnet made 2 small slips in 28 answers and followed the rules, vacancies included.
- Sonnet is slower: first word after about 1.2 s (Haiku 0.65 s), a whole answer in 2–8 s (Haiku 1–5 s), measured locally through the gateway.

Cost: about $0.023 per answer (~7,500 input tokens of CV, `about.md` and instructions plus ~250 output tokens; Sonnet 5's tokenizer counts about 30% more tokens for the same text than older models), measured at $0.86 for 28 Sonnet and 24 Haiku answers. 500 answers a month is about $12; Haiku would be about $4.5.

**Thinking stays off.** Sonnet 5 thinks by default (adaptive thinking), and thinking tokens count toward `maxOutputTokens`: with it on, answers were cut off at the limit and the first word came much later. The chat sends `providerOptions.anthropic.thinking: { type: "disabled" }`; the gateway applies it on all four providers (checked on 2026-09-13: a question that cost 1,650–1,925 thinking tokens by default cost 0 with this setting, on each provider). Sonnet 5 also rejects non-default sampling parameters, so the chat sets no `temperature`.

Cheaper candidates on the gateway, roughly 4–5× cheaper per answer than Haiku (benchmarks as of September 2026):
...
- No prompt caching yet. The fixed part of the prompt (CV, `about.md`, rules) is the same for every request, so caching it would make later questions in a conversation cheaper and faster; see "Later".
...
9. **Length and format:** the UI shows plain text, so Markdown would appear as literal symbols. Never use Markdown, even though `<cv>` and `<about>` do: no bold or italics, no headings, no tables, no bold labels in front of items. Usually two to four sentences, at most about 100 words, longer only when the visitor asks for detail. A list only when it really helps, one item per line starting with "- ". Models do not always follow this, so the UI also removes bold markers (see "Layout and copy").
...
- [ ] `www.overeem.io` redirects to `https://overeem.io/`; a CAA record allows only Let's Encrypt; DNSSEC validates
- [ ] Live on overeem.io
...
- Prompt caching of the fixed part of the prompt (CV, `about.md`, rules), if the gateway passes Anthropic's cache setting through to all four providers: cached input costs a tenth, and a later question in the same conversation would start answering sooner
